...
class Double_Gaussian_V2_Network(keras.Model):
    ...
    def call(self, state: tf.Tensor) -> collections.namedtuple:
        ...
        with tf.compat.v1.name_scope('MeanPatch'):
            ...
            action_mean = tf.reshape(action_mean, [batch_size, self.action_num])[:, :, None] - 1

            # BatchSize x [ActionNum * 1]
            action_hv_mean = self.s_dense_2_mean_2(self.s_dense_2_mean_1(state_hv_feature_vec))
            # BatchSize x ActionNum x 1
            action_hv_mean = tf.reshape(action_hv_mean, [batch_size, self.action_num])[:, :, None] + 1
        ...
        with tf.compat.v1.name_scope('STDPatch'):
            ...
            action_std = self.dense_2_std_2(self.dense_2_std_1(state_feature_vec))
            # BatchSize x ActionNum x 1
            action_std = tf.reshape(action_std, [batch_size, self.action_num])[:, :, None] / 5
            #action_std = tf.exp(action_std)
            action_std = tf.math.softplus(action_std)

            # BatchSize x [ActionNum * 1]
            action_hv_std = self.s_dense_2_std_2(self.s_dense_2_std_1(state_hv_feature_vec))
            # BatchSize x ActionNum x 1
            action_hv_std = tf.reshape(action_hv_std, [batch_size, self.action_num])[:, :, None] / 5
            #action_hv_std = tf.exp(action_hv_std)
            action_hv_std = tf.math.softplus(action_hv_std)
...
class DoubleGaussianAgent(rainbow_agent.RainbowAgent):
    def __init__(self, sess, action_num, network=Double_Gaussian_V2_Network, double_dqn: bool = True,
                 sample_num: int = 128, action_mode: str = 'mean',
    ...
    def _build_samples_op(self, _target_mean, _target_std):
        batch_size = self._replay.batch_size
        half_sample_num = self.sample_num // 2
    ...
    def _build_target_statistic_op(self, _cur_mean, _cur_std):
        ...
        # BatchSize x SampleNum
        dist_1_mean = tf.tile(_cur_mean[:, 0:1], multiples=[1, self.sample_num])
        dist_2_mean = tf.tile(_cur_mean[:, 1:2], multiples=[1, self.sample_num])
        # BatchSize x SampleNum
        dist_1_std = tf.tile(_cur_std[:, 0:1], multiples=[1, self.sample_num])
        dist_2_std = tf.tile(_cur_std[:, 1:2], multiples=[1, self.sample_num])
        ...
        prob_1 = tf.clip_by_value(prob_1, 0.001, 1)
        prob_2 = tf.clip_by_value(prob_2, 0.001, 1)
        prob_weight = prob_1 / (prob_1 + prob_2)

        #prob_weight = tf.clip_by_value(prob_weight, 0.005, 0.995)
        prob_weight = prob_weight / tf.reduce_mean(prob_weight, axis=1, keepdims=True) * 0.5

        random_matrix = tf.compat.v1.random.uniform(shape=[batch_size, self.sample_num], minval=0.001, maxval=0.999)

        # BatchSize x SampleNum
        group_1_mask_matrix = tf.cast(random_matrix < prob_weight, tf.float32)
        group_2_mask_matrix = 1 - group_1_mask_matrix
    ...
    def _build_train_op(self):
        ...
        with tf.control_dependencies([update_priorities_op]):
            ...
            #mean_train_op = mean_optimizer.minimize(tf.reduce_mean(mean_loss))
            mean_train_op = mean_optimizer.minimize(tf.reduce_mean(mean_loss), var_list=mean_net_params)

            std_optimizer = tf.compat.v1.train.AdamOptimizer(0.0000425, epsilon=0.00015)
            #std_train_op = std_optimizer.minimize(tf.reduce_mean(std_loss))
            std_train_op = std_optimizer.minimize(tf.reduce_mean(std_loss), var_list=std_net_params)
