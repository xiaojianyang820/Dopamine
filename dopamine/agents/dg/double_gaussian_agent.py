...
class BaseConvResNetwork(keras.Model):
    def __init__(self, name: str = None):
        ...
        self.flatten = keras.layers.Flatten()
        self.conv_1_1 = keras.layers.Conv2D(32, [5, 5], strides=1, kernel_initializer=kernel_initializer,
...
class DoubleGaussianResNetwork(keras.Model):
    ...
    def __init__(self, action_num: int, name: str = None, embedding_dim: int = 128):
        ...
        self.res_1 = BaseConvResNetwork('Res_1')
        self.res_2 = BaseConvResNetwork('Res_2')
        self.res_3 = BaseConvResNetwork('Res_3')
        self.flatten = keras.layers.Flatten()
        self.dense_1_mean_1 = keras.layers.Dense(action_num * 5, kernel_initializer=kernel_initializer,
                                                 activation=activation_fn, name='MeanFeature')
        self.dense_1_mean_2 = keras.layers.Dense(action_num * 2, kernel_initializer=kernel_initializer,
    ...
    def call(self, state):
        ...
        # BatchSize x ActionNum x 2
        action_mean = tf.reshape(mean, [batch_size, self.action_num, 2])

        # BatchSize x [ActionNum * 2]
        std = self.dense_2_mean_2(self.dense_2_mean_1(res_feature))
        # BatchSize x ActionNum x 2
        action_std = tf.reshape(std, [batch_size, self.action_num, 2]) / 2
        action_std = tf.exp(action_std)

        return self.DoubleGaussianNetworkType(action_mean=action_mean, action_std=action_std)
...
class DoubleGaussianAgent(rainbow_agent.RainbowAgent):
    def __init__(self, sess, action_num, network=Double_Gaussian_V2_Network, double_dqn: bool = True,
                 ...
                 loss_mode: str = 'mse', is_double_EM: bool = False,
        ...
        self.is_double_EM = is_double_EM
    ...
    def _build_target_statistic_op(self, _cur_mean, _cur_std):
        ...
        prob_1 = tf.clip_by_value(prob_1, 1e-15, 3)
        prob_2 = tf.clip_by_value(prob_2, 1e-15, 3)
        ...
        group_1_mean = sample_1_mean[:, None]
        #group_1_mean = dist_1_mean
        sample_1_std = tf.reduce_sum((target_samples - group_1_mean) ** 2 * group_1_mask_matrix, axis=1) / \
                       tf.reduce_sum(group_1_mask_matrix, axis=1)
        ...
        group_2_mean = sample_2_mean[:, None]
        #group_2_mean = dist_2_mean
        sample_2_std = tf.reduce_sum((target_samples - group_2_mean) ** 2 * group_2_mask_matrix, axis=1) / \
                       tf.reduce_sum(group_2_mask_matrix, axis=1)
        ...
        batch_index = tf.tile(tf.range(0, batch_size)[:, None], [1, 2])
    ...
    def _build_train_op(self):
        ...
        if self.is_double_EM:
            print('进行两次EM迭代')
            # BatchSize x 2
            _tar_mean, _tar_std = self._build_target_statistic_op(_tar_mean, _tar_std)
        ...
        with tf.control_dependencies([update_priorities_op]):
            if self.loss_mode == 'mse':
                ...
                mean_optimizer = tf.compat.v1.train.AdamOptimizer(0.0000625, epsilon=0.00015)
                mean_train_op = mean_optimizer.minimize(tf.reduce_mean(kl_loss * 0.5 + w2_loss))
    ...
    def _train_step(self):
        ...
        if self._replay.memory.add_count > self.min_replay_history:
            if self.training_steps % self.update_period == 0:
                _, _, mean_loss, std_loss, kl_loss, cur_mean, cur_std, tar_mean, tar_std, = self._sess.run(self._train_op)
