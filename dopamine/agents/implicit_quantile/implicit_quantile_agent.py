...
class ImplicitQuantileAgent(rainbow_agent.RainbowAgent):
  ...
  def __init__(self,
               ...
               double_dqn=False,
               before_project_mix_samples: bool = False,
               summary_writer=None,
  ...
  def _build_target_quantile_values_op(self):
    ...
    # (num_tau_prime_samples x batch_size) x 1
    bellmaned_quantile_values = rewards + gamma_with_terminal * target_quantile_values
    no_mixed_quantile_values = bellmaned_quantile_values
    # BatchSize x NumTauPrime x 1
    bellmaned_quantile_values = tf.transpose(
        tf.reshape(bellmaned_quantile_values, [self.num_tau_prime_samples, batch_size, 1]), [1, 0, 2]
    )
    ...
    # BatchSize x [NumTau+NumTauPrime] x 1
    mixed_quantile_values = tf.concat([bellmaned_quantile_values, chosen_action_quantile_values], axis=1)
    # ([NumTau+NumTauPrime] x BatchSize) x 1
    mixed_quantile_values = tf.reshape(
        tf.transpose(mixed_quantile_values, [1, 0, 2]),
        [(self.num_tau_prime_samples+self.num_tau_samples)*batch_size, 1]
    )
    if self.before_project_mix_samples:
        print('在投影操作前进行新旧样本混合')
        return mixed_quantile_values
    else:
        print('在投影操作后不进行新旧样本混合')
        return no_mixed_quantile_values
  ...
  def _build_train_op(self):
    ...
    num_mixed_tau_samples = target_quantile_values.get_shape().as_list()[0] // batch_size
    ...
    replay_quantiles = tf.cast(
        tf.tile(replay_quantiles[:, None, :, :],
                [1, num_mixed_tau_samples, 1, 1]), tf.float32)
