...
FQFNetworkType = collections.namedtuple(
  'fqf_network', [
    'quantile_values', 'quantiles',
    'v_diff', 'L_tau', 'gradient_tau', 'quantile_tau'
  ]
)
...
@gin.configurable
class FQFNetwork(keras.Model):
  def __init__(self, num_actions: int, quantile_embedding_dim: int, num_quantiles: int,
               name: str = None):
    super(FQFNetwork, self).__init__(name=name)
    self.num_actions = num_actions
    self.quantile_embedding_dim = quantile_embedding_dim
    ...
    self.dense_1_4 = keras.layers.Dense(quantile_embedding_dim, kernel_initializer=kernel_initializer,
                                        activation=None, name='StateFeature_FC')

    self.dense_2_1 = keras.layers.Dense(num_quantiles, activation=None, kernel_initializer=kernel_initializer,
                                        name='Quantile_FC')

    self.dense_3_1 = keras.layers.Dense(quantile_embedding_dim, activation=activation_fn,
                                        kernel_initializer=kernel_initializer, name='QuantileFeature_FC')

    self.dense_4_1 = keras.layers.Dense(512, activation=activation_fn, kernel_initializer=kernel_initializer,
                                        name='QuantileValue_FC_4_1')
    self.dense_4_2 = keras.layers.Dense(num_actions, activation=None, kernel_initializer=kernel_initializer,
                                        name='QuantileValue_FC_4_2')
  ...
  def call(self, state: tf.Tensor) -> collections.namedtuple:
    ...
    :param state: tf.Tensor,
        形状为BatchSize x Height x Width x Depth
    ...
    with tf.compat.v1.name_scope('StateFeaturePatch'):
      ...
      # BatchSize x QuantileEMDim
      state_vec = self.dense_1_4(self.flatten(state))
    # 状态观测特征向量 -->> 分位点序列 区块
    with tf.compat.v1.name_scope('StateToQuantilePatch'):
      base_quantile = 0.025
      # BatchSize x QuantileNum
      quantiles_right_inner = self.dense_2_1(tf.stop_gradient(state_vec))
      # 将提议出来的分位点变量进行Softmax变换，以解决规范性问题
      quantiles_right_inner = tf.nn.softmax(quantiles_right_inner, axis=1) * (1 - 2 * base_quantile)
      # 对序列进行累加，得到真正的分位点序列（右侧序列）
      # BatchSize x QuantileNum
      quantiles_right = tf.cumsum(quantiles_right_inner, axis=1) + base_quantile
      # 在分位点序列（右侧序列）的最左侧添加一个0，得到分位点序列的完备序列
      zeros = tf.zeros([batch_size, 1]) + base_quantile
      # BatchSize x [QuantileNum + 1]
      quantiles_all = tf.concat([zeros, quantiles_right], axis=1)
      # 分位点序列的左侧序列
      # BatchSize x QuantileNum
      quantiles_left = quantiles_all[:, :-1]
      # 分位点序列的右侧序列
      # BatchSize x QuantileNum
      quantiles_right = quantiles_all[:, 1:]
      # 分位点序列的中段序列
      # BatchSize x [QuantileNum - 1]
      quantiles_center = quantiles_all[:, 1:-1]
      # 分位点序列的区段中心点序列
      # BatchSize x QuantileNum
      quantiles_mid = tf.stop_gradient((quantiles_right + quantiles_left) / 2)
      # 分位点序列的区段长度序列
      # BatchSize x QuantileNum
      v_diff = quantiles_right - quantiles_left
      # QuantileNum x BatchSize
      v_diff_t = tf.transpose(v_diff, [1, 0])
      # [QuantileNum - 1] x BatchSize
      quantiles_center_t = tf.transpose(quantiles_center, [1, 0])

    with tf.compat.v1.name_scope('QuantileFeaturePatch'):
      # [(QuantileNum - 1) x BatchSize] x 1
      quantiles_center_flatten = tf.reshape(quantiles_center_t, [-1, 1])
      # [(QuantileNum - 1) x BatchSize] x QuantileEMDim
      quantiles_center_tiled = tf.tile(quantiles_center_flatten, [1, self.quantile_embedding_dim])
      # 对分位点序列的中段序列进行余弦编码和非线性编码
      pi = tf.constant(math.pi)
      quantiles_center_pi = tf.cast(tf.range(1, self.quantile_embedding_dim + 1, 1), tf.float32) \
                            * pi * quantiles_center_tiled
      quantiles_center_cos = tf.cos(quantiles_center_pi)
      # [(QuantileNum - 1) x BatchSize] x QuantileEMDim
      quantiles_center_vec = self.dense_3_1(quantiles_center_cos)

      # QuantileNum x BatchSize
      quantiles_mid_t = tf.transpose(quantiles_mid, [1, 0])
      # [QuantileNum x BatchSize] x 1
      quantiles_mid_flatten = tf.reshape(quantiles_mid_t, [-1, 1])
      # [QuantileNum x BatchSize] x QuantileEMDim
      quantiles_mid_tiled = tf.tile(quantiles_mid_flatten, [1, self.quantile_embedding_dim])
      # 对分位点序列的区段中心点序列进行余弦编码和非线性编码
      pi = tf.constant(math.pi)
      quantiles_mid_pi = tf.cast(tf.range(1, self.quantile_embedding_dim + 1, 1), tf.float32) \
                         * pi * quantiles_mid_tiled
      # [QuantileNum x BatchSize] x QuantileEMDim
      quantiles_mid_cos = tf.cos(quantiles_mid_pi)
      # [QuantileNum x BatchSize] x QuantileEMDim
      quantiles_mid_vec = self.dense_3_1(quantiles_mid_cos)

    with tf.compat.v1.name_scope('QuantileMapPatch'):
      # [(QuantileNum - 1) x BatchSize] x QuantileEMDim
      center_state_vec_tiled = tf.tile(state_vec, [self.num_quantiles - 1, 1])
      # [(QuantileNum - 1) x BatchSize] x QuantileEMDim
      center_concated_vec = tf.multiply(center_state_vec_tiled, quantiles_center_vec)
      # [(QuantileNum - 1) x BatchSize] x 512
      center_concated_vec = tf.stop_gradient(self.dense_4_1(center_concated_vec))
      # [(QuantileNum - 1) x BatchSize] x ActionNum
      center_quantile_values = tf.stop_gradient(self.dense_4_2(center_concated_vec))

      # [Quantile_num x BatchSize] x QuantileEMDim
      mid_state_vec_tiled = tf.tile(state_vec, [self.num_quantiles, 1])
      # [Quantile_num x BatchSize] x QuantileEMDim
      mid_concated_vec = tf.multiply(mid_state_vec_tiled, quantiles_mid_vec)
      # [Quantile_num x BatchSize] x 512
      mid_concated_vec = self.dense_4_1(mid_concated_vec)
      # [Quantile_num x BatchSize] x ActionNum
      mid_quantile_values = self.dense_4_2(mid_concated_vec)

    # [QuantileNum - 1] x BatchSize x ActionNum
    center_quantile_values = tf.reshape(center_quantile_values,
                                        [self.num_quantiles - 1, batch_size, self.num_actions])
    # QuantileNum x BatchSize x ActionNum
    mid_quantile_values = tf.reshape(mid_quantile_values,
                                     [self.num_quantiles, batch_size, self.num_actions])
    # [QuantileNum - 1] x BatchSize x ActionNum
    mid_1_quantile_values = mid_quantile_values[:-1, :, :]
    mid_2_quantile_values = mid_quantile_values[+1:, :, :]

    diff_1 = center_quantile_values - mid_1_quantile_values
    diff_2 = mid_2_quantile_values - center_quantile_values
    gradient_tau = diff_1 - diff_2

    # [QuantileNum - 1] x BatchSize x ActionNum
    L_tau = tf.square(gradient_tau)

    return FQFNetworkType(quantile_values=tf.reshape(mid_quantile_values, [-1, self.num_actions]),
                          quantiles=quantiles_mid_flatten,
                          v_diff=v_diff_t, L_tau=L_tau, gradient_tau=gradient_tau,
                          quantile_tau=quantiles_center_t)
