...
class FourierBasis(object):
  ...
  Fourier基线性函数近似
  每一个维度上都要求一个等差序列，可以在上面使用正弦或者余弦变换。因此它只有完整Fourier近似变换所使用参数的一半参数
  """

  def __init__(self, nvars: int, min_vals: int = 0, max_vals: int = None, order: int = 3):
    """
    初始化函数
  ...
  def scale(self, values: tf.Tensor):
  ...
  def compute_features(self, features: tf.Tensor):
...
class FourierDQNNetwork(tf.keras.Model):
  ...
  def __init__(self, min_vals, max_vals, num_actions, fourier_basis_order=3,
    ...
    构建一个函数逼近器来计算一个智能体的Q值
    它使用Fourier基特征和不带偏置的线性层
